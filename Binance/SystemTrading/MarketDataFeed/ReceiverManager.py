...
from SystemTrading.MarketDataFeed.Public.StreamReceiverWebsocket import StreamReceiverWebsocket
from SystemTrading.MarketDataFeed.Public.KlineReceiverWebsocket import KlineReceiverWebsocket
from SystemTrading.MarketDataFeed.Private.ExecutionReceiverWebsocket import ExecutionReceiverWebsocket
from SystemTrading.MarketDataFeed.Public.KlineCycleFetcher import KlineCycleFetcher
from SystemTrading.MarketDataFeed.Public.OrderBookCycleFetcher import OrderbookCycleFechter
...
class ReceiverManager:
    def __init__(self,
        ...
        self.ws_ticker = StreamReceiverWebsocket("ticker", self.queue_ticker, self.event_loop_status)
        self.ws_trade = StreamReceiverWebsocket("trade", self.queue_trade, self.event_loop_status)
        self.ws_minTicker = StreamReceiverWebsocket("minTicker", self.queue_minTicker, self.event_loop_status)
        self.ws_depth = StreamReceiverWebsocket("depth", self.queue_depth, self.event_loop_status)
        self.ws_aggTrade = StreamReceiverWebsocket("aggTrade", self.queue_aggTrade, self.event_loop_status)
        self.ws_kline = KlineReceiverWebsocket(self.queue_kline_ws, self.event_loop_status)
        self.ws_execution = ExecutionReceiverWebsocket(self.queue_execution_ws, self.event_executuion_ws)
        self.fetcher_kline = KlineCycleFetcher(self.queue_kline_fetcher, self.loop_status)
        self.fetcher_orderbook = OrderbookCycleFechter(self.queue_orderbook_fetcher)
