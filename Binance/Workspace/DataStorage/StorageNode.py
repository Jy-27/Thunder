...
class SubStorageOverwrite:
    ...
    def __deepcopy__(self, memo):
        ...
        """
        new_copy = SubStorageOverwrite(self._fields)  # 새로운 인스턴스 생성
...
class MainStorageOverwrite:
    """
    💾 메인 저장소의 field에 저장될 보조 저장소다.
    Node Tree를 구현하기 위함이다.
    """

    def __new__(cls, fields: List[str], sub_storage: SubStorageOverwrite):
        """
        동적으로 __slots__을 설정하고, SubStorageOverwrite를 저장하는 구조.
        """
        new_cls = type(cls.__name__, (cls,), {"__slots__": tuple(fields)})
        instance = super().__new__(new_cls)
    ...
    def __init__(self, fields: List[str], sub_storage: SubStorageOverwrite):
        for field in fields:
    ...
    def clear_all(self):#, main_field: str):
        ...
        Raises:
            ValueError: 필드명(속성명)이 존재하지 않을 때
        """
        for main_field in self.__slots__:
    ...
    def to_dict(self, main_field:str) -> Dict:
        """
        📤 MainStorageOverwrite와 SubStorageOverwrite 지정 필드정보를 Dictionary타입으로 구성하여 반환한다.
...
class SubStorageAppend:
    ...
    def __deepcopy__(self, memo):
        ...
        deepcopy 지원: 새 인스턴스를 만들고 기존 값들을 복사.
        """
        new_copy = SubStorageAppend(self._fields)  # 새로운 인스턴스 생성
        for field in self._fields:
            setattr(new_copy, field, deepcopy(getattr(self, field), memo))
        return new_copy
...
class MainStorageAppend:
    """
    💾 메인 저장소의 field에 저장될 보조 저장소다.
    Node Tree를 구현하기 위함이다.
    """

    def __new__(cls, fields: List[str], sub_storage: SubStorageAppend):
        """
        동적으로 __slots__을 설정하고, SubStorageOverwrite를 저장하는 구조.
        """
        new_cls = type(cls.__name__, (cls,), {"__slots__": tuple(fields)})
    ...
    def __init__(self, fields: List[str], sub_storage: SubStorageAppend):
        for field in fields:
            setattr(self, field, deepcopy(sub_storage))
...
if __name__ == "__main__":
    ...
    sub_storage = SubStorageOverwrite(sub_fields)

    # ✅ 메인 저장소 생성
    main_fields = SystemConfig.Streaming.symbols
    main_storage = MainStorageOverwrite(main_fields, sub_storage)
