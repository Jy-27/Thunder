...
class TradeManager(ABC):
    ...
    def position_size(cls, symbol:str, mark_price:float, leverage:int, balance:float, test_mode:bool=False) -> float:
        ...
        symbol_detail = futures_trading_utils.Extractor.symbol_detail(symbol, exchange_data)
        filter_data = futures_trading_utils.Extractor.symbol_filters(symbol_detail['filters'])
    ...
    @classmethod
    def check_leverage(cls, symbol: str, leverage: int, test_mode: bool) -> int:
        """
        ...
        Returns:
            int: 적용가능한 레버리지값
        """
        brackets_data = futures_trading_utils.Selector.brackets_data(symbol, test_mode)        
        max_leverage = futures_trading_utils.Extractor.max_leverage(brackets_data)
