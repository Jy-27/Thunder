...
class TradingLog:
    ...
    stoploss: Optional[float] = None  # 포지션 종료 가격 지정.
    ...
    def __post_init__(self):
        ...
        if self.last_time is None:
            self.last_time = self.start_timestamp
        # 진입가가 0이하면 오류발생
...
class StopLoss:
    ...
    def __init__(
        ...
        adjust_rate: Optional[float] = 0.0025,
        adjust_interval: str = "5m",
    ):
        # 초기 포지션 종료 비율
    ...
    def update_trading_log(self, symbol: str, current_price: float):
        """
        ...
        self.trading_log[symbol].update_trade_data(
            current_price=current_price, current_time=current_timestamp
        )
        # 종료 가격을 계산한다.
