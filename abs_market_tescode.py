...
class AbstractClass(ABC):
    ...
    async def _fetch_data(self, endpoint: str, params: Optional[Dict[str, Any]] = None) -> Optional[Any]:
        """공통 API 호출 메서드"""
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, params=params, timeout=10) as response:
                    if response.status == HTTPStatus.OK:
                        ...
                        logging.error(f"HTTP 오류: {response.status} - {response.reason}")
                        return None
...
class SpotMarketData(AbstractClass):
    ...
    async def get_klines(self, symbol: str, interval: str, limit: int = 100):
        if interval not in self.OHLCV_INTERVALS:
            raise ValueError(f"유효하지 않은 간격: {interval}. 사용 가능한 간격: {self.OHLCV_INTERVALS}")
