...
class DummyDataGenerator:
    ...
    async def get_historical_kline(
        ...
        3. MEMO
            >> 본 함수는 interval데이터를 수집할뿐 각 interval 데이터간 연결성이 없다. idx값을 매칭처리할 수는 있으나,
            백테스트 리얼데이터로 활용하기에는 부적합하다. 각 interval 데이터를 동시에 평가하면서 이동평균선을 사용시 의미없는 데이터만 계산될 뿐이다.
        """
        symbol = symbol.upper()
        ...
        for interval in intervals:
            historical_data[interval] = (
                await self.instance.fetch_historical_kline_hour_min(
                    symbol=symbol,
                    interval=interval,
    ...
    def merge_json_files(
        self, directory_path: str, file_suffix: str
    ) -> Dict[str, Dict]:
        """
        ...
        for file_name in os.listdir(directory_path):
            if file_name.endswith(file_suffix):
                ...
                if not isinstance(json_data, dict):
                    raise ValueError(
                        f"파일 '{file_name}'의 데이터가 딕셔너리 형식이 아닙니다."
                    )
...
class IntervalKlineExtractor:
    ...
    # 리얼 데이터 삽입시 필수 데이터.!!
    # 수집된 kline데이터를 활용하여 1분봉(또는 min interval)기준으로 리얼 데이터를 생성한다. ()
    def generate_real_time_kline(
        self,
        indices_data: Optional[List[List[int]]] = None,
        kline_data: Optional[Dict[str, Dict[str, NDArray[np.float64]]]] = None,
    ):
        """
        ...
        for symbol, kline_data_symbol in kline_data.items():
            ...
            for interval, kline_data_interval in kline_data_symbol.items():
                ...
                if interval == target_interval:
                    processed_data[symbol][interval] = np.array(kline_data_interval)
                    continue
                    ...
                    if idx == 0:
                        ...
                        if prev_open_timestamp == open_timestamp:
                            updated_kline = [
                                open_timestamp,
                                previous_kline[1],
                                max(previous_kline[2], current_data[2]),
                                min(previous_kline[3], current_data[3]),
                                current_data[4],
                                previous_kline[5] + current_data[5],
                                previous_kline[6],
                                previous_kline[7] + current_data[7],
                                previous_kline[8] + current_data[8],
                                previous_kline[9] + current_data[9],
                                previous_kline[10] + current_data[10],
                                0,  # 예시 데이터, 필요시 수정
                            ]
                            processed_data[symbol][interval].append(updated_kline)  # 마지막 항목 업데이트
                        else:
                            processed_data[symbol][interval].append(new_kline)
...
def data_sync():
    ...
    for symbol in symbols:
        ...
        path = os.path.join(directory, symbol + "_" + suffix)
        data = asyncio.run(
            ins_generator.get_historical_kline(
                symbol=symbol,
                start_date=start_date,
                end_date=end_date,
                intervals=intervals,
    ...
    merge_data = ins_generator.merge_json_files(
        directory_path=directory, file_suffix=suffix
    )
    utils._save_to_json(file_path=path_data, new_data=merge_data, overwrite=True)
...
def process_symbol(symbol, i, kline_data):
    ...
    # 구매 신호 조건
    if (
        scenario_1[2] >= 3 and scenario_1[3] >= 3 and scenario_1[-1]
    ):  # and scenario_1[2]>=3 and scenario_1[4] > 0:#  and (scenario_1[2] > 2 or scenario_1[3]) > 2 and scenario_1[4] > 0:
        print(scenario_1)
        index_ = indices_data[i][0]
...
if __name__ == "__main__":
    ...
    ins_kline_ex = IntervalKlineExtractor(
        kline_data=merge_data, index_data=indices_data, symbols=symbols
    )
    # data_length = ins_kline_ex.get_data_length()
    data_range = 60 * 24
    ...
    for idx in range(0, len(indices_data) - data_range, 1):
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            futures = [
                executor.submit(process_symbol, symbol, i, kline_data)
                for symbol in symbols
            ]
